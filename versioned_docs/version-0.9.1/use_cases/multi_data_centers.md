...
Here we use TiDB cluster as an example. Suppose we already install the [TiDB cluster](https://docs.pingcap.com/tidb-in-kubernetes/stable/) and [Chaos Mesh](../installation/installation.md) in our Kubernetes environment. In this TiDB cluster, we have three TiDB pods, three PD pods and seven TiKV pods:
...
Chaos Mesh provides [`NetworkChaos`](../user_guides/networkchaos.md) to inject network latency, so we can use it to simulate the latency between three data centers.
