...
Because of the `Go Template` conflict with `helm`, the common template is not included in the [helm chart](../installation/installation.md#install-by-helm). However, it will be deployed automatically if you install Chaos Mesh via the [install script](../installation/installation.md#install-chaos-mesh).
